...
export function ContactButtons({ contact, title }: ContactButtonsProps) {
  const { phone, whatsapp, telegram } = contact;

  if (!phone && !whatsapp && !telegram) {
    return (
      <p className="text-sm text-gray-400 italic">Контакты не указаны</p>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-2">
      {/* Телефон */}
      {phone && (
        <a href={buildPhoneLink(phone)} className="block">
      ...
      {whatsapp && (
        <a
          href={buildWhatsAppLink(whatsapp, title ? `Здравствуйте! Вас интересует объявление "${title}"?` : undefined)}
          target="_blank"
