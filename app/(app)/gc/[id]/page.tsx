...
export default async function GcReport({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ from?: string; to?: string }> }) {
  ...
  const items = d.items.map(e => ({ id: e.id, vendor: e.vendor || e.category, date: e.spent_on, amount: e.amount, src: e.receipt_path ? `/api/file?path=${encodeURIComponent(e.receipt_path)}` : null, reimbursed: !!e.reimbursed_on }))
  ...
  return (
    <div className="lg:max-w-[720px]">
      ...
      <GcList items={items} />
      <GcActions siteId={id} from={from} to={to} outstandingIds={d.items.filter(e => !e.reimbursed_on).map(e => e.id)} />
    </div>
  )
}
